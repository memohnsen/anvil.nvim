...
describe("Forge topic view", function()
  ...
  it("renders stored topic metadata", function()
    ...
    -- Title bar (kind/number/state on one line, title on the next).
    assert.True(rendered:find("Pull request #42", 1, true) ~= nil)
    assert.True(rendered:find("● OPEN", 1, true) ~= nil)
    assert.True(rendered:find("Add a topic buffer", 1, true) ~= nil)
    -- Keybinding hint bar.
    assert.True(rendered:find("refresh", 1, true) ~= nil)
    assert.True(rendered:find("comment", 1, true) ~= nil)
    -- Details fields and section headers.
    assert.True(rendered:find("@mona", 1, true) ~= nil)
    assert.True(rendered:find("feature", 1, true) ~= nil)
    assert.True(rendered:find("+1 2, heart 1", 1, true) ~= nil)
    assert.True(rendered:find("topic-buffer", 1, true) ~= nil)
    assert.True(rendered:find("mona, team-reviewers", 1, true) ~= nil)
    ...
    assert.True(rendered:find("@octocat", 1, true) ~= nil)
    assert.True(rendered:find("rocket 3", 1, true) ~= nil)
    assert.True(rendered:find("Looks useful.", 1, true) ~= nil)
    assert.True(rendered:find("@hubot", 1, true) ~= nil)
    assert.True(rendered:find("APPROVED", 1, true) ~= nil)
    assert.True(rendered:find("Ship it.", 1, true) ~= nil)
    assert.True(rendered:find("▌ Review threads", 1, true) ~= nil)
    assert.True(rendered:find("lua/anvil/forge/init.lua:88 (unresolved, outdated)", 1, true) ~= nil)
    assert.True(rendered:find("eyes 1", 1, true) ~= nil)
    assert.True(rendered:find("@@ -1,2 +1,2 @@", 1, true) ~= nil)
    assert.True(rendered:find("Can we keep the old behavior here?", 1, true) ~= nil)
    assert.True(rendered:find("lua/anvil/forge/init.lua:88-88", 1, true) ~= nil)
    assert.True(rendered:find("▌ Pending review comments", 1, true) ~= nil)
    assert.True(rendered:find("lua/anvil/forge/init.lua:99", 1, true) ~= nil)
  ...
  it("queues pending review comments and submits pull request reviews", function()
    ...
    local rendered = table.concat(view.buffer:get_lines(0, -1), "\n")
    assert.True(rendered:find("lua/anvil/forge/init.lua:23", 1, true) ~= nil)
