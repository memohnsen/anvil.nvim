...
-- IMPORTANT: Verify this require path matches the actual location in Anvil's source
local Hunk = require("anvil.buffers.common").Hunk
...
describe("Hunk Component", function()
  it("correctly highlights standard context lines starting with a minus", function()
    ...
    -- "  foo:" -> Pure context
    eq("AnvilDiffContext", lines[1].options.line_hl)

    -- " -- tricky context with minus" -> SHOULD BE CONTEXT NOW (Fixes the bug)
    eq("AnvilDiffContext", lines[2].options.line_hl)

    -- " +- tricky context with plus" -> SHOULD BE CONTEXT
    eq("AnvilDiffContext", lines[3].options.line_hl)

    -- "- deleted line" -> Actual deletion
    eq("AnvilDiffDelete", lines[4].options.line_hl)

    -- "+ added line" -> Actual addition
    eq("AnvilDiffAdd", lines[5].options.line_hl)
  end)
  ...
  it("correctly highlights combined diff lines (merge conflicts)", function()
    ...
    -- " -- deleted in both parents"
    eq("AnvilDiffDelete", lines[2].options.line_hl)

    -- " ++ added in both parents"
    eq("AnvilDiffAdd", lines[3].options.line_hl)

    -- " +- added parent 1, deleted parent 2"
    eq("AnvilDiffAdd", lines[4].options.line_hl)
  end)
end)
