local TopicView = require("neogit.buffers.forge_topic_view")
...
describe("Forge topic view", function()
  ...
  it("renders stored topic metadata", function()
    forge.pending_review_comments = function()
      return {
        {
          path = "lua/neogit/forge/init.lua",
          line = 99,
    ...
    local view = TopicView.new {
      ...
      head = "topic-buffer",
      base = "main",
      url = "https://github.com/NeogitOrg/neogit/pull/42",
    ...
    assert.True(rendered:find("Thread 1: lua/neogit/forge/init.lua:88 (unresolved, outdated)", 1, true) ~= nil)
    ...
    assert.True(rendered:find("Suggestion 1.1.1: lua/neogit/forge/init.lua:88-88", 1, true) ~= nil)
    assert.True(rendered:find("Pending Review Comments:", 1, true) ~= nil)
    assert.True(rendered:find("Pending 1: lua/neogit/forge/init.lua:99", 1, true) ~= nil)
  ...
  it("queues pending review comments and submits pull request reviews", function()
    ...
    local prompts = { "lua/neogit/forge/init.lua", "23" }
    ...
    local rendered = table.concat(view.buffer:get_lines(0, -1), "\n")
    assert.True(rendered:find("Pending 1: lua/neogit/forge/init.lua:23", 1, true) ~= nil)
