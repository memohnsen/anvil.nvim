...
local queries = require("neogit.forge.queries")
...
function M.comment_topic(topic, body, cb)
  ...
  if not cli then
    notification.warn("Forge: commenting on discussions from Neogit is not implemented yet")
    if cb then
      cb(false, "unsupported topic kind")
    end
    return
...
function M.edit_topic_title(topic, title, cb)
  local cli = topic_cli(topic)
  if not cli then
    notification.warn("Forge: editing discussions from Neogit is not implemented yet")
    if cb then
      cb(false, "unsupported topic kind")
    end
    return
...
function M.toggle_topic_state(topic, cb)
  local cli = topic_cli(topic)
  if not cli then
    notification.warn("Forge: closing/reopening discussions from Neogit is not implemented yet")
    if cb then
      cb(false, "unsupported topic kind")
    end
    return
