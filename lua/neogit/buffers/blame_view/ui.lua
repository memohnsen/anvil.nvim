...
local function HunkHeading(hunk)
  ...
  if hunk.uncommitted then
    children = {
      text(hunk.abbrev, { highlight = "NeogitObjectId" }),
      text(" "),
      text("Uncommitted changes", { highlight = "NeogitSubtleText" }),
    }
  else
    children = {
      text(hunk.abbrev, { highlight = "NeogitObjectId" }),
      text(" "),
      text(hunk.author, { highlight = "NeogitGraphAuthor" }),
      text(" "),
      text(M.relative_date(hunk.author_time), { highlight = "NeogitSubtleText" }),
      text(" "),
      text(hunk.summary),
    }
  end

  return row(children, { line_hl = "NeogitDiffHeader" })
end
