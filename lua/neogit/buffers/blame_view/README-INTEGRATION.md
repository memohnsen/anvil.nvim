...
into neogit, the following is needed:

## Entry point

Add a `blame` sub-command to the `:Neogit` command dispatcher so users can run
it from a file buffer:

```lua
-- e.g. in lua/neogit.lua / the command dispatcher:
blame = function()
  local file = vim.api.nvim_buf_get_name(0)
  if file == "" then
    require("neogit.lib.notification").error("Buffer is not backed by a file")
    return
  end
...
`:Neogit blame`, or a popup entry in the future.

## Public API

```lua
local BlameView = require("neogit.buffers.blame_view")
...
require("neogit.lib.git").blame.run(file, rev) -- data layer; returns BlameHunk[]|nil, err
```
