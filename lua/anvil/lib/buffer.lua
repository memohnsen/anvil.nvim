...
local diff_highlights = require("anvil.lib.diff_highlights")
...
function Buffer:new(handle, win_handle)
  local this = {
    autocmd_group = api.nvim_create_augroup("Anvil-augroup-" .. handle, { clear = true }),
    handle = handle,
    ...
    name = nil,
    namespaces = {
      default = api.nvim_create_namespace("anvil-buffer-" .. handle),
    },
  }
...
function Buffer:set_line_highlights(highlights)
  if vim.b[self.handle] and vim.b[self.handle].anvil_disable_hunk_highlight == true then
    return
  end
...
function Buffer:show()
  ...
  local function open()
    ...
    elseif self.kind == "popup" then
      local title, _ = self.name:gsub("^Anvil", ""):gsub("Popup$", "")
...
function Buffer:create_namespace(name)
  ...
  local namespace = "anvil-buffer-" .. self.handle .. "-" .. name
...
function Buffer:get_namespace_id(name)
  ...
  if name and name ~= "default" then
    ns_id = self.namespaces["anvil-buffer-" .. self.handle .. "-" .. name]
  else
...
function Buffer:set_header(text, scroll)
  ...
  vim.wo[winid].winhl = "NormalFloat:AnvilFloatHeader"

  fn.matchadd("AnvilFloatHeaderHighlight", [[\v\<cr\>|\<esc\>]], 100, -1, { window = winid })
...
function Buffer.create(config)
  ...
  if win then
    ...
    vim.api.nvim_set_hl(hl_ns, "FoldColumn", { link = "AnvilFoldColumn" })
    vim.api.nvim_set_hl(hl_ns, "SignColumn", { link = "AnvilSignColumn" })
    vim.api.nvim_set_hl(hl_ns, "Normal", { link = "AnvilNormal" })
    vim.api.nvim_set_hl(hl_ns, "NormalFloat", { link = "AnvilNormalFloat" })
    vim.api.nvim_set_hl(hl_ns, "FloatBorder", { link = "AnvilFloatBorder" })
    vim.api.nvim_set_hl(hl_ns, "WinSeparator", { link = "AnvilWinSeparator" })
    vim.api.nvim_set_hl(hl_ns, "CursorLineNr", { link = "AnvilCursorLineNr" })
    vim.api.nvim_set_hl(hl_ns, "CursorLine", { link = "AnvilCursorLine" })
  ...
  if config.context_highlight then
    ...
    buffer:set_decorations("ViewContext", {
      ...
      on_win = function()
        ...
        local disable_hl = vim.b.anvil_disable_hunk_highlight == true

        for line = start, stop do
          local is_cursor = line == cursor
          if is_cursor or not disable_hl then
            local line_hl = ("%s%s"):format(
              buffer.ui:get_line_highlight(line) or "AnvilDiffContext",
  ...
  if config.active_item_highlight then
    ...
    buffer:set_decorations("ActiveItem", {
      ...
      on_win = function()
        ...
        local active_oid = require("anvil.buffers.commit_view").current_oid()
        ...
        if item and item.first and item.last then
          for line = item.first, item.last do
            buffer:add_line_highlight(line - 1, "AnvilActiveItem", {
              priority = 200,
        ...
        for line = fn.line("w0"), fn.line("w$") do
          if foldmarkers[line] then
            ...
            buffer:place_sign(line, fold .. string.lower(foldmarkers[line]), {
              namespace = "FoldSigns",
              highlight = "AnvilSubtleText",
            })
          else
            buffer:place_sign(line, "AnvilBlank", {
              namespace = "FoldSigns",
            })
          end
        end
