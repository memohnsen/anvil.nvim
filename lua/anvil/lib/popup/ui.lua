...
local function construct_config_options(config, prefix, suffix)
  ...
  local options = filter_map(config.options, function(option)
    ...
    if config.value == option.value then
      set = true
      highlight = "AnvilPopupConfigEnabled"
    else
      highlight = "AnvilPopupConfigDisabled"
    end

    return text.highlight(highlight)(option.display)
  end)

  local value = intersperse(options, text.highlight("AnvilPopupConfigDisabled")("|"))
  table.insert(value, 1, text.highlight("AnvilPopupConfigDisabled")("["))
  table.insert(value, #value + 1, text.highlight("AnvilPopupConfigDisabled")("]"))

  if prefix then
    table.insert(
      value,
      1,
      text.highlight(set and "AnvilPopupConfigEnabled" or "AnvilPopupConfigDisabled")(prefix)
    )
  end
  ...
  if suffix then
    table.insert(
      value,
      #value + 1,
      text.highlight(set and "AnvilPopupConfigEnabled" or "AnvilPopupConfigDisabled")(suffix)
    )
  end
...
local function get_highlight_for_switch(switch)
  if switch.enabled then
    return "AnvilPopupSwitchEnabled"
  end

  return "AnvilPopupSwitchDisabled"
end
...
local function get_highlight_for_option(option)
  if option.value ~= nil and option.value ~= "" then
    return "AnvilPopupOptionEnabled"
  end

  return "AnvilPopupOptionDisabled"
end
...
local function get_highlight_for_config(config)
  if config.value and config.value ~= "" then
    return config.type or "AnvilPopupConfigEnabled"
  end

  return "AnvilPopupConfigDisabled"
end
...
local Switch = Component.new(function(switch)
  ...
  return row.tag("Switch").value(switch)({
    row.highlight("AnvilPopupSwitchKey") {
...
local Option = Component.new(function(option)
  return row.tag("Option").value(option)({
    row.highlight("AnvilPopupOptionKey") {
...
local Section = Component.new(function(title, items)
  return col {
    text.highlight("AnvilPopupSectionTitle")(title),
    col(items),
  }
end)
...
local Config = Component.new(function(props)
  ...
  if not props.state[1].heading then
    table.insert(c, text.highlight("AnvilPopupSectionTitle")("Variables"))
  end
  ...
  table.insert(
    ...
    col(map(props.state, function(config)
      if config.heading then
        return row.highlight("AnvilPopupSectionTitle") { text(config.heading) }
      end
      ...
      return row.tag("Config").value(config)({
        row.highlight("AnvilPopupConfigKey") { text(key) },
        text(" " .. config.name .. " "),
...
local function render_action(action)
  ...
  elseif #action.keys == 0 then
    table.insert(items, text.highlight("AnvilPopupActionDisabled")("_"))
  else
    for i, key in ipairs(action.keys) do
      table.insert(items, text.highlight("AnvilPopupActionKey")(key))
    ...
    Grid.padding_left(1) {
      ...
      render_item = function(item)
        if item.heading then
          return row.highlight("AnvilPopupSectionTitle") { text(item.heading) }
        elseif not item.callback then
          return row.highlight("AnvilPopupActionDisabled")(render_action(item))
        else
