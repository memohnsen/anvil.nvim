...
function M.apply(buf, regions)
  ...
  local ns = buf:create_namespace("AnvilDiffHighlight")
          ...
          while i <= n do
            ...
            for j = 0, math.min(del_count, add_count) - 1 do
              ...
              if distance <= MAX_DISTANCE then
                apply_spans(buf_lines[del_start + j], old_spans, "AnvilDiffDeleteInline")
                apply_spans(buf_lines[add_start + j], new_spans, "AnvilDiffAddInline")
              end
            end
