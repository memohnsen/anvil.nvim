--- Coroutine-based async library for Anvil.
---
--- Drop-in replacement for the subset of plenary.async used by Anvil:
...
local function execute(task, async_fn, callback, ...)
  ...
  step = function(...)
    ...
    if type(fn) ~= "function" or type(argc) ~= "number" then
      local err = "[anvil async] coroutine yielded an unexpected value; "
...
function M.wrap(fn, argc)
  ...
  return function(...)
    ...
    if not in_async_context() then
      error(
        "[anvil async] wrapped function called outside an async context "
    ...
    task:on_complete(function(cancelled, ok, err, tb)
      if not ok and not cancelled then
        vim.schedule(function()
          error("[anvil async] " .. (tb or tostring(err) or "unknown error"), 0)
        end)
      end
    end)
  end
...
function M.util.block_on(async_fn, timeout)
  ...
  if not outcome then
    task:cancel()
    error("[anvil async] block_on timed out after " .. (timeout or 2000) .. " ms")
  end
