...
local record = require("anvil.lib.record")
...
M.graph = util.memoize(function(options, files, color)
  ...
  return util.filter_map(result, function(line)
    return require("anvil.lib.ansi").parse(util.trim(line), { recolor = not color })
  end)
end)
...
M.list = util.memoize(function(options, graph, files, hidden, graph_color)
  ...
  if graph then
    if config.values.graph_style == "unicode" then
      graph_output = require("anvil.lib.graph.unicode").build(commits)
    elseif config.values.graph_style == "kitty" then
      graph_output = require("anvil.lib.graph.kitty").build(commits, graph_color)
...
function M.register(meta)
  meta.update_recent = function(repo_state)
    ...
    local order = state.get({ "AnvilMarginPopup", "-order" }, config.values.commit_order)
...
---@param remotes string[] list of remote names, e.g. by calling `require("anvil.lib.git").remote.list()`
