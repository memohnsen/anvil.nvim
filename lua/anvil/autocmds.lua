...
function M.setup()
  ...
  local status_buffer = require("anvil.buffers.status")
  local git = require("anvil.lib.git")
  local group = require("anvil").autocmd_group

  api.nvim_create_autocmd({ "ColorScheme" }, {
    callback = function()
      local config = require("anvil.config")
      local highlight = require("anvil.lib.hl")
  ...
  api.nvim_create_autocmd({ "BufWritePost", "ShellCmdPost", "VimResume" }, {
    callback = a.void(function(o)
      if
        ...
        and not api.nvim_get_option_value("filetype", { buf = o.buf }):match("^Anvil")
      then
