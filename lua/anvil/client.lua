local RPC = require("anvil.lib.rpc")
...
function M.get_nvim_remote_editor(show_diff)
  local anvil_path = debug.getinfo(1, "S").source:sub(2, -#"lua/anvil/client.lua" - 2)
  local nvim_path = fn.shellescape(vim.v.progpath)

  logger.debug("[CLIENT] Anvil path: " .. anvil_path)
  logger.debug("[CLIENT] Neovim path: " .. nvim_path)
  local runtimepath_cmd = fn.shellescape(fmt("set runtimepath^=%s", fn.fnameescape(tostring(anvil_path))))
  local lua_cmd =
    fn.shellescape("lua require('anvil.client').client({ show_diff = " .. tostring(show_diff) .. " })")
...
function M.get_envs_git_editor(show_diff)
  ...
  if os.getenv("ANVIL_DEBUG") then
    env.ANVIL_LOG_LEVEL = "debug"
    env.ANVIL_LOG_FILE = "true"
    env.ANVIL_DEBUG = true
  end
...
function M.client(opts)
  ...
  local lua_cmd = fmt('lua require("anvil.client").editor(%q, %q, %s)', file_target, client, opts.show_diff)
...
function M.editor(target, client, show_diff)
  ...
  require("anvil.process").hide_preview_buffers()
  ...
  if target:find("git%-rebase%-todo$") then
    editor = require("anvil.buffers.rebase_editor")
  else
    editor = require("anvil.buffers.editor")
  end
