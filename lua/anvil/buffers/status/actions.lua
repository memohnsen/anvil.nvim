...
local jump = require("anvil.lib.jump")
...
M.n_close = function(self)
  return require("anvil.lib.ui.helpers").close_topmost(self)
end
...
M.n_open_or_scroll_down = function(self)
  return function()
    ...
    if commit then
      require("anvil.buffers.commit_view").open_or_scroll_down(commit)
    end
  end
end
...
M.n_open_or_scroll_up = function(self)
  return function()
    local commit = self.buffer.ui:get_commit_under_cursor()
    if commit then
      require("anvil.buffers.commit_view").open_or_scroll_up(commit)
    end
  end
end
...
M.n_command_history = function(_self)
  return a.void(function()
    require("anvil.buffers.git_command_history"):new():show()
  end)
end
...
M.n_show_refs = function(_self)
  return a.void(function()
    require("anvil.buffers.refs_view").new(git.refs.list_parsed(), git.repo.worktree_root):open()
  end)
end
    ...
    if stagable and section then
      ...
      if selection.item and selection.item.mode == "UU" then
        ...
        if diff_viewer and git.merge.is_conflicted(selection.item.escaped_path) then
          local integration = diff_viewer == "codediff" and require("anvil.integrations.codediff")
            or require("anvil.integrations.diffview")
...
---Opens anvil on the parent repo if if we are in a submodule
---@param self StatusBuffer
...
M.n_goto_file = function(self)
  return function()
    ...
    if item and (item.kind == "pullreq" or item.kind == "issue") then
      require("anvil.buffers.forge_topic_view").new(item):open()
      return
    end
    ...
    if item and item.absolute_path then
      if self:has_submodule(item.absolute_path) then
        self:close()
        require("anvil").open { cwd = item.absolute_path }
        return
      end
    ...
    if ref then
      require("anvil.buffers.commit_view").new(ref):open()
    end
  end
end
...
M.n_command = function(self)
  ...
  local runner = require("anvil.runner")
