...
local ui = require("anvil.buffers.status.ui")
...
function M:has_submodule(abs_path)
  local dir = require("anvil.lib.path"):new(abs_path)
...
function M:_action(name)
  local action = require("anvil.buffers.status.actions")[name]
...
function M:open(kind)
  ...
  self.buffer = Buffer.create {
    ...
    after = function(buffer, _win)
      ...
      vim.b.anvil_git_dir = git.repo.git_dir
    end,
    user_autocmds = {
      ...
      ["AnvilReset"] = self:deferred_refresh("reset"),
      ["AnvilBranchReset"] = self:deferred_refresh("reset_branch"),
      ["AnvilEditorClosed"] = self:deferred_refresh("editor_closed"),
    },
    autocmds = {
...
function M:chdir(dir)
  ...
  vim.schedule(function()
    ...
    require("anvil.lib.git.repository").instance(dir)
