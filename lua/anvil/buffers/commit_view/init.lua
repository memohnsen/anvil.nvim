...
local parser = require("anvil.buffers.commit_view.parsing")
local ui = require("anvil.buffers.commit_view.ui")
...
local commit_view_maps = require("anvil.config").get_reversed_commit_view_maps()
...
local function is_jumpable_hunk_line_component(c)
  return c.options.line_hl == "AnvilDiffContext"
    or c.options.line_hl == "AnvilDiffAdd"
    or c.options.line_hl == "AnvilDiffDelete"
end
...
function M:open(kind)
  ...
  self.buffer = Buffer.create {
    name = "AnvilCommitView",
    filetype = "AnvilCommitView",
    kind = kind,
    ...
    mappings = {
      n = {
        ...
        ["<cr>"] = function()
          local c = self.buffer.ui:get_component_under_cursor(function(c)
            return c.options.highlight == "AnvilFilePath" or is_jumpable_hunk_line_component(c)
          end)
          ...
          -- Recursively navigate the layout until we hit AnvilDiffHeader leaf nodes
          ...
          local function find_diff_headers(layout)
            if layout.children then
              ...
              if layout.options.line_hl == "AnvilDiffHeader" then
