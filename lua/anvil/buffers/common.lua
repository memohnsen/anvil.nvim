...
M.Diff = Component.new(function(diff)
  return col.tag("Diff")({
    text(string.format("%s %s", diff.kind, diff.file), { line_hl = "AnvilDiffHeader" }),
    M.DiffHunks(diff),
...
M.Hunk = Component.new(function(props)
  return col.tag("Hunk")({
    text.line_hl("AnvilHunkHeader")(props.header),
...
local function build_graph(graph, opts)
  ...
  if type(graph) == "table" then
    return util.map(graph, function(g)
      ...
      return text(char, { highlight = string.format("AnvilGraph%s", g.color) })
    end)
  else
...
local highlight_for_signature = {
  G = "AnvilSignatureGood",
  B = "AnvilSignatureBad",
  U = "AnvilSignatureGoodUnknown",
  X = "AnvilSignatureGoodExpired",
  Y = "AnvilSignatureGoodExpiredKey",
  R = "AnvilSignatureGoodRevokedKey",
  E = "AnvilSignatureMissing",
  N = "AnvilSignatureNone",
}
  ...
  if args.decorate and commit.ref_name ~= "" then
    ...
    for name, _ in pairs(info.locals) do
      ...
      elseif info.remotes[name] == nil then
        local branch_highlight = info.head == name and "AnvilBranchHead" or "AnvilBranch"
  ...
  if args.details then
    ...
    details = col.padding_left(#commit.abbreviated_commit + 1) {
      row(util.merge(graph, {
        text(" "),
        text("Author:     ", { highlight = "AnvilSubtleText" }),
        text(commit.author_name, { highlight = "AnvilGraphAuthor" }),
        text(" <"),
      ...
      row(util.merge(graph, {
        text(" "),
        text("AuthorDate: ", { highlight = "AnvilSubtleText" }),
        text(commit.author_date),
      })),
      row(util.merge(graph, {
        text(" "),
        text("Commit:     ", { highlight = "AnvilSubtleText" }),
      ...
      row(util.merge(graph, {
        text(" "),
        text("CommitDate: ", { highlight = "AnvilSubtleText" }),
      ...
      col(
        ...
        { highlight = "AnvilCommitViewDescription" }
      ),
    }
  end
