...
local highlights = {
  local_branch = "AnvilBranch",
  remote_branch = "AnvilRemote",
  tag = "AnvilTagName",
  ["+"] = "AnvilGraphCyan",
  ["-"] = "AnvilGraphPurple",
  ["<>"] = "AnvilGraphYellow",
...
local function Cherries(ref, head)
  local cherries = util.map(git.cherry.list(head, ref.oid), function(cherry)
    return row({
      ...
      text(" "),
      text.highlight("AnvilObjectId")(cherry.oid:sub(1, git.log.abbreviated_size())),
      text(" "),
      text.highlight("AnvilGraphWhite")(cherry.subject),
    }, { oid = cherry.oid })
  end)
...
local function Ref(ref)
  local ref_content = {
    text.highlight("AnvilGraphBoldPurple")(ref.head and "@ " or "  "),
...
local function section(refs, heading, head)
  ...
  return col.tag("Section")({
    row.tag("SectionHeading")(
      util.merge(heading, { text.highlight("AnvilGraphWhite")(string.format(" (%d)", #refs)) })
    ),
...
function M.Branches(branches, head)
  return { section(branches, { text.highlight("AnvilBranch")("Branches") }, head) }
end
...
function M.Remotes(remotes, head)
  ...
  for _, name in pairs(sorted_names(remotes)) do
    ...
    table.insert(
      out,
      section(branches, {
        text.highlight("AnvilBranch")("Remote "),
        text.highlight("AnvilRemote")(name, { align_right = max_len }),
        text.highlight("AnvilBranch")(
...
function M.Tags(tags, head)
  return { section(tags, { text.highlight("AnvilBranch")("Tags") }, head) }
end
